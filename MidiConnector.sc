MidiConnector : AbstractConnector {
	...
	*new { |widget, name|
	...
	getMidiMode {
		^this.widget.wmc.midiOptions.m.value[this.index].midiMode;
	}
	...
	getMidiZero {
		^this.widget.wmc.midiOptions.m.value[this.index].midiZero;
	}
	...
	getMidiSnapDistance {
		^this.widget.wmc.midiOptions.m.value[this.index].snapDistance;
	}
	...
	getMidiCtrlButtonGroup {
		^this.widget.wmc.midiOptions.m.value[this.index].ctrlButtonGroup;
	}
	...
	getMidiResolution {
		^this.widget.wmc.midiOptions.m.value[this.index].midiResolution;
	}
	...
	getMidiInputMapping {
		^this.widget.wmc.midiOptions.m.value[this.index].midiInputMapping;
	}
	...
	getMidiTemplate {
		^this.widget.wmc.midiDisplay.m.value[this.index].template.interpret;
	}
	...
	getMidiDispatcher {
		^this.widget.wmc.midiDisplay.m.value[this.index].dispatcher.interpret;
	}
	...
	prMIDIFunc { |index, cc, ch, src, t, d|
		...
		if (learn) {
			...
			makeFunc.().learnSync(this.widget, index);
		} {
	...
	remove { |forceAll = false|
		...
		if (mc.midiConnectors.m.value.size > 1 or: { forceAll }) {
			this.midiDisconnect;
			// allMidiFuncs[this.widget][index].free;
			// allMidiFuncs[this.widget].removeAt(index);
			[
